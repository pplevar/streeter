...
- **Fail fast + signal (workers):** a shared `SyncOutcomeHandler` (`domain/sync/`) is the one unit that reacts to a sync outcome. On a `SyncAuthException` it returns `FAILURE` (so the worker fails on the first attempt, no backoff) and raises the user signal; on any other failure it retries until `WorkRetryPolicy`'s budget is spent; on success it clears the signal. The three sync workers delegate to it instead of each duplicating the retry fold. Issue #60 widened it from "should I retry?" to "what does this outcome mean?": it also lands the `SYNC_FAILED` status after a failed push and triggers the pull that follows a successful one.
- **User signal:** `SyncAuthStatus` (`domain/sync/`) holds an observable `authFailed` flag. Settings observes it and shows a banner above the token field ("Sync was rejected — set or correct your token"); editing the token clears it, and the next attempt re-raises it only if the new token is still rejected.
...
- **Classify the `401` inside each worker** (inspect the failure and branch on retry). Rejected: duplicated across three workers and hard to unit-test (a `CoroutineWorker` needs a WorkManager/Android harness this project doesn't use). A pure `SyncOutcomeHandler` + `SyncAuthStatus` puts the fail-fast and signal-raising behavior behind plain JVM tests; the workers become thin delegators.
