...
- **Distinguish 401 from transient failures and surface auth errors to the user.** Deferred to a follow-up issue (sync silently rests in `SYNC_FAILED` today); safe because prod stays `PERMISSIVE` until the manual `STRICT` flip.
