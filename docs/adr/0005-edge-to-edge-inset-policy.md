...
- `EditPointsScreen` reads one value, `WindowInsets.navigationBars`, and derives the sheet's painted height, the list's bottom `contentPadding`, and the camera's bottom padding from it. Its Scaffold content `PaddingValues` parameter is deliberately unused.
...
- There is no automated test for this. The repo has no `androidTest` source set or Compose UI test dependency, and the expected pixel values vary by device and navigation mode. Verification is a screenshot plus a colour-band pixel scan locating element bounds, cross-checked against `uiautomator` node bounds — the method used to diagnose the second defect, and the prior art for future inset work.
