...
class PendingMatchJobRepositoryImpl @Inject constructor(
    ...
    override suspend fun getJobForWalk(walkId: Long): PendingMatchJob? =
        dao.getJobForWalk(walkId)?.toDomain()
    ...
    private fun PendingMatchJob.toEntity() = PendingMatchJobEntity(
        id = id, walkId = walkId, queuedAt = queuedAt,
        status = status.name, retryCount = retryCount, lastError = lastError
    )

    private fun PendingMatchJobEntity.toDomain() = PendingMatchJob(
        id = id, walkId = walkId, queuedAt = queuedAt,
        status = JobStatus.valueOf(status), retryCount = retryCount, lastError = lastError
    )
}
